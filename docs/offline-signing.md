...
| `input.txType` | `string` | Transaction type (`transfer`, `addOwner`, `removeOwner`, `changeThreshold`, `setDelegate`, `createChild`, `reclaimChild`, `destroyChild`, `enableChildMultiSig`) |
...
### `createChild` — propose deploys, execute initializes

With the deploy-at-propose-time flow, `createChild` is fully supported offline:

- **Propose bundle** includes `input.childPrivateKey`, `input.childOwners`, and `input.childThreshold`. The CLI generates the child keypair's deploy account update and signs it with the bundled private key, then submits `propose()` on the parent — all in a single transaction. The child private key is generated by the UI at export time and included in the bundle so the air-gapped machine can sign the deploy.
- **Approve bundle** is standard — no special handling beyond including the child account snapshot for nonce freshness checks.
- **Execute bundle** includes the child account snapshot and child events (same as other REMOTE lifecycle actions). The CLI calls `executeSetupChild` on the already-deployed child. No child private key is needed at this stage.

**Security note:** The bundle containing `input.childPrivateKey` should be treated as sensitive material during transport to the air-gapped machine. However, the child private key is only useful for signing the deploy account update — once the child is deployed, the key has no further power (all state changes require ZK proofs, not signatures).
